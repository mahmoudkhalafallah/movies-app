...
margin-top: 10;
margin-bottom: 10;
${({ isMyMovie, isFirst }: any) =>
	isMyMovie && isFirst ? 'margin-left: 25' : 'margin-left: 10'}
${({ isMyMovie, isLast }: any) =>
	isMyMovie && isLast ? 'margin-right: 20' : 'margin-right: 10'}
...
export const Movie: React.FC<Props> = ({
	...
	return (
		<MovieContainer isFirst={isFirst} isLast={isLast} isMyMovie={isMyMovie}>
			<MovieImage
				src={isMyMovie ? poster_path : `${IMAGE_URI_PREFIX}${poster_path}`}
				isMyMovie={isMyMovie}
			/>
			<MovieDetails style={{ width: isMyMovie ? 100 : 150 }}>
				<MovieTitle ellipsizeMode="tail" numberOfLines={1}>
					{title}
				</MovieTitle>
				{vote_average != undefined && (
					<MovieRating rating={vote_average} hideRatingValue={isMyMovie} />
				)}
				{!isMyMovie && (
					<>
						<MovieDate>{release_date}</MovieDate>
						<MovieOverview ellipsizeMode="tail" numberOfLines={6}>
							{overview}
						</MovieOverview>
					</>
				)}
