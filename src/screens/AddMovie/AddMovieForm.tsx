...
import { View, ScrollView, Button, Platform, TouchableHighlight, Alert, Image, ImageSourcePropType, Text } from "react-native";
...
export const AddMovieForm: React.FC<{ onSubmit: (data: any) => void, setModalVisibility: (visibility: boolean) => void }> = ({ onSubmit, setModalVisibility }) => {
    ...
    const [posterSource, setPosterSource]: [ImageSourcePropType, any] = useState({ uri: undefined });
    ...
    const openImagePicker = () => ImagePicker.launchImageLibrary(options, (response) => {
    ...
    return <ScrollView style={{ marginHorizontal: 15 }}>
        <FormHeader>Add your Movie</FormHeader>

        <View>
            <FormField>Movie Title</FormField>
            <StyledInput ref={register({ name: 'title' }, { required: true })} onChangeText={text => setValue('title', text, true)} />
            {errors.title && <ErrMsg>Movie Title is required</ErrMsg>}

            <FormField>Movie Overview</FormField>
            <StyledInput multiline numberOfLines={4} ref={register({ name: 'overview' }, { required: true })} onChangeText={text => setValue('overview', text, true)} />
            ...
            <TouchableHighlight onPress={show} underlayColor="transparent">
                <>
                    ...
                    <StyledInput ref={register({ name: 'release_date' }, { required: true })} value={`${d.getFullYear()}-${d.getMonth()}-${d.getDate()}`} editable={false} />
                    {errors.release_date && <ErrMsg>Movie Date is required</ErrMsg>}
                </>
            </TouchableHighlight>

            {pickerData.show && <DateTimePicker value={pickerData.date}
                display="default"
                onChange={setDate} />
            }
            ...
            <Button title="Choose Poster" onPress={openImagePicker} ref={register({ name: 'poster_path' }, { required: true })}></Button>
        ...
        <ButtonsContainer>
            <Button title='Add movie' color={Variables.colorPrimary} onPress={handleSubmit(onSubmit)} />
            <Button
                color='gray'
                title='Cancel'
                onPress={() => {
                    Alert.alert(
                        ...
                        [
                            { text: 'Yes', onPress: () => { setModalVisibility(false) } },
                            { text: 'No', onPress: () => { } }
                        ]
                    )
